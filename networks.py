...
class _HFVideoTransformerClassifier(nn.Module):
    def __init__(
        self,
        architecture,
        num_classes,
        ...
        try:
            ...
            raise ImportError(
                "transformers is required for VideoMAE/TimeSformer backbones."
            ) from exc

        self.architecture = architecture
        self.tune_mode = tune_mode
        model_cls = VideoMAEForVideoClassification if architecture == "videomae" else TimesformerForVideoClassification
        ...
        self.target_size = self._resolve_target_size(image_size)
        self.input_mean = torch.tensor([0.485, 0.456, 0.406], dtype=torch.float32).view(1, 1, 3, 1, 1)
        self.input_std = torch.tensor([0.229, 0.224, 0.225], dtype=torch.float32).view(1, 1, 3, 1, 1)
        self.model_mean = self._resolve_processor_stats("image_mean", [0.45, 0.45, 0.45])
        self.model_std = self._resolve_processor_stats("image_std", [0.225, 0.225, 0.225])

        if self.tune_mode == "linear_probe":
    ...
    def _resolve_processor_stats(self, key, default):
        values = getattr(self.processor, key, default)
        return torch.tensor(values, dtype=torch.float32).view(1, 1, 3, 1, 1)

    def _resolve_target_size(self, fallback):
        if hasattr(self.processor, "crop_size") and isinstance(self.processor.crop_size, dict):
            height = self.processor.crop_size.get("height", fallback)
            width = self.processor.crop_size.get("width", fallback)
    ...
    def _prepare_inputs(self, x):
        x = x.float()
        input_mean = self.input_mean.to(device=x.device, dtype=x.dtype)
        input_std = self.input_std.to(device=x.device, dtype=x.dtype)
        model_mean = self.model_mean.to(device=x.device, dtype=x.dtype)
        model_std = self.model_std.to(device=x.device, dtype=x.dtype)

        x = x * input_std + input_mean
        x = x.clamp(0.0, 1.0)

        if x.shape[1] != self.target_frames:
            temporal = x.permute(0, 2, 1, 3, 4)
            temporal = F.interpolate(
                temporal,
                size=(self.target_frames, x.shape[3], x.shape[4]),
                mode="trilinear",
                align_corners=False,
            )
            x = temporal.permute(0, 2, 1, 3, 4)

        if x.shape[-2:] != self.target_size:
            b, t, c, h, w = x.shape
            x = x.reshape(b * t, c, h, w)
            x = F.interpolate(x, size=self.target_size, mode="bilinear", align_corners=False)
            x = x.reshape(b, t, c, self.target_size[0], self.target_size[1])
    ...
    def embed(self, x):
        pixel_values = self._prepare_inputs(x)
        outputs = self.model.videomae(pixel_values=pixel_values) if self.architecture == "videomae" else self.model.timesformer(pixel_values=pixel_values)
...
class VideoMAEClassifier(_HFVideoTransformerClassifier):
    def __init__(self, num_classes, model_name_or_path, frames=16, image_size=224, tune_mode="linear_probe"):
        super().__init__(
...
class TimeSformerClassifier(_HFVideoTransformerClassifier):
    def __init__(self, num_classes, model_name_or_path, frames=16, image_size=224, tune_mode="linear_probe"):
        super().__init__(
