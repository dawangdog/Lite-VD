...
class singleKinetics400(tdata.Dataset):
    def __init__(self, path, split, transform):
        ...
        print(
            f"{split}: {len(self.video_dirs)} samples, Skipped {item_to_skip} items "
            f"(expected_frames={self.num_frames}, frame_root={self.frame_root})"
        )
...
class miniUCF101(tdata.Dataset):
    ...
    def get_all_frames(self, index):
        ...
        for i in range(1, length+1):
            image = Image.open(
                os.path.join(path, "img{:05d}.jpg".format(i))
            )
            image = self.transform(image)
            X.append(image)
        X = torch.stack(X, dim=0)
...
class SSv2(tdata.Dataset):
    def __init__(self, path, split, transform, num_frames=16):
        self.num_frames = int(num_frames)
        self.transform = transform
        self.split = split

        json_path = osp.join(path, f"annot_{split}.json")
        frame_root = osp.join(path, "frame")
        rawframe_root = osp.join(path, "rawframes")
        if osp.exists(frame_root):
            self.frame_root = frame_root
        elif osp.exists(rawframe_root):
            self.frame_root = rawframe_root
        else:
            self.frame_root = frame_root
        ...
        with open(json_path) as fp:
            ...
            for item in content:
                ...
                sample_dir = osp.join(self.frame_root, name)
                if (
                    not osp.exists(sample_dir)
                    or len(os.listdir(sample_dir)) != self.num_frames
                ):
                    item_to_skip += 1
                    print("skip", name)
                else:
...
if __name__ == "__main__":
    ...
    ssv2 = SSv2(
        "./distill_utils/data/SSv2","train",transform=transform
    )
    _ssv2 = ssv2[3007]
    print("label:", _ssv2[1])
